...
const Tooltip = ({ children, isHovering }: { children: React.ReactNode; isHovering: boolean }) => {
  ...
  return (
    <div
      className={`${kiwiSoda.className} absolute right-12 bottom-0 z-20 flex w-2xl flex-col gap-2 rounded-xl bg-[#d0d995] p-4 text-center text-2xl font-medium text-[#b8555b]`}
    >
      {children}
    </div>
  );
